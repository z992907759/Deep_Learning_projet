...
def main():
    ...
    out_dir = ensure_dir(project_root / "outputs" / "runs" / "O2")
    ...
    model = O2SameResTwinViT(
        backbone=cfg["model"]["backbone"],     # 两分支同 backbone => 同分辨率
    ...
    # 记录曲线
    history_epochs, history_train_loss, history_val_acc, history_val_f1 = [], [], [], []
    ...
    for epoch in range(1, int(cfg["train"]["epochs"]) + 1):
        ...
        for batch in tr_loader:
            x1 = batch["raw"].to(device)  # raw
            x2 = batch["seg"].to(device)  # seg
            y = batch["y"].to(device)
        ...
        # 评估输入也用 raw+seg（等价于 O1 的 C1 输入形式）
        metrics = evaluate(model, va_loader, device, mode="C1")
        ...
        print(
            f"[O2] epoch={epoch:02d} loss={avg_loss:.4f} "
    ...
    plt.xlabel("epoch"); plt.ylabel("train loss"); plt.title("O2 Train Loss")
    ...
    plt.xlabel("epoch"); plt.ylabel("val acc"); plt.title("O2 Val Accuracy")
    plt.grid(True, linestyle="--", linewidth=0.5)
    plt.tight_layout()
    ...
    plt.xlabel("epoch"); plt.ylabel("val f1"); plt.title("O2 Val F1")
    plt.grid(True, linestyle="--", linewidth=0.5)
    plt.tight_layout()
